...
if(is_array($arResult) && count($arResult)>0) {
	?><div class="catalogmenucolumn"><?
		?><ul class="catalogmenu list-unstyled clearfix"><?
			?><li class="parent"><a href="<?=$arParams['RSGOPRO_CATALOG_PATH']?>" class="parent">Все разделы<?//=GetMessage('RSGOPRO_CATALOG')?><?
			?><img class="svg-icon menu" src="<?=$templateFolder?>/images/list.svg"><!--svg class="svg-icon menu"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-menu"></use></svg--></a><?
...
//if($USER->IsAdmin()) {echo '<pre>'; print_r($max); echo '</pre>';}
			$last_lvl1 = false;
			foreach($arResult as $arItem){
				...
				if ($arItem['IS_PARENT']) {
					if ($arItem['DEPTH_LEVEL'] == 1) {
						?><li class="first<?if ($index>$max):?> more<?endif;?><?if($arItem['IS_LAST_LVL1']=='Y'):?> lastchild<?endif;?><?=(!empty($arItem['DETAIL_PICTURE']) ? ' catalogmenu-icon' : '')?>"><?
							?><a href="<?=$arItem['LINK']?>" class="first<?if($arItem['SELECTED']):?> selected<?endif?>" title="<?=$arItem['TEXT']?>"><?
								...
								?><img class="svg-icon arrow" src="../../images/right-arrow.svg"><!--svg class="svg-icon arrow"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-arrow-linear-right"></use></svg--><?
							?></a><?
					...
					if ($arItem['DEPTH_LEVEL'] == 1) {
						...
						<li class="sub<?if($arItem['SELECTED']):?> selected<?endif?>">
							<a href="<?=$arItem['LINK']?>" class="sub" title="<?=$arItem['TEXT']?>"><?=$arItem['TEXT']?></a>
						</li><?
					}
				}
