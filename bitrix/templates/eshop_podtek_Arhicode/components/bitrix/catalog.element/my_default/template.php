...
$i = 1;
	while ($obPHOTO_ACCESSORIES = $resPHOTO_ACCESSORIES->GetNext()) {	if(!empty($obPHOTO_ACCESSORIES['VALUE'])){$isACCESSORIES = true;}	}
	while ($obACCESSORIES = $resACCESSORIES->GetNext()) {	if(!empty($obACCESSORIES['VALUE'])){$isACCESSORIES = true;}	}

$resVIDEOYOUTUBE = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"VIDEO_YOUTUBE"));
	while ($obVideoYoutube = $resVIDEOYOUTUBE->GetNext()) {	if(!empty($obVideoYoutube['VALUE'])) {$isVIDEOYOUTUBE = true;}	}

$resINSTRUCTIONS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"INSTRUCTIONS"));
	while ($obINSTRUCTIONS = $resINSTRUCTIONS->GetNext()) {	if(!empty($obINSTRUCTIONS['VALUE'])){$isINSTRUCTIONS = true;}	}

$resCERTIFICATES = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"CERTIFICATES"));
	while ($obCERTIFICATES = $resCERTIFICATES->GetNext()) {	if(!empty($obCERTIFICATES['VALUE'])){$isCERTIFICATES = true;};	
...
$resDIMENSIONS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"DIMENSIONS"));
	while ($obDIMENSIONS = $resDIMENSIONS->GetNext()) {	if(!empty($obDIMENSIONS['VALUE'])) {$isDIMENSIONS = true;};		}
...
$resRELATEDPRODUCTS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"RELATED_PRODUCTS"));
	while ($obRELATEDPRODUCTS = $resRELATEDPRODUCTS->GetNext()) {	if(!empty($obRELATEDPRODUCTS['VALUE'])) {$isRELATEDPRODUCTS = true;};		}
...
$resH2ATTRIBUTES = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_ATTRIBUTES"));
	while ($obH2ATTRIBUTES = $resH2ATTRIBUTES->GetNext()) {	if(!empty($obH2ATTRIBUTES['VALUE'])) {$isH2ATTRIBUTES = true;}	}
...
$resH2DESCRIPTION = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_DESCRIPTION"));
	while ($obH2DESCRIPTION = $resH2DESCRIPTION->GetNext()) {	if(!empty($obH2DESCRIPTION['VALUE'])) {$isH2DESCRIPTION = true;}	}
...
$resH2DIMENSIONS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_DIMENSIONS"));
	while ($obH2DIMENSIONS = $resH2DIMENSIONS->GetNext()) {	if(!empty($obH2DIMENSIONS['VALUE'])) {$isH2DIMENSIONS = true;}	}
...
$resH2ACCESSORIES = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_ACCESSORIES"));
	while ($obH2ACCESSORIES = $resH2ACCESSORIES->GetNext()) {	if(!empty($obH2ACCESSORIES['VALUE'])) {$isH2ACCESSORIES = true;}	}
...
$resH2RELATEDPRODUCTS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_RELATED_PRODUCTS"));
	while ($obH2RELATEDPRODUCTS = $resH2RELATEDPRODUCTS->GetNext()) {	if(!empty($obH2RELATEDPRODUCTS['VALUE'])) {$isH2RELATEDPRODUCTS = true;}	}
...
$resH2SECTIONVIDEO = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_VIDEO"));
	while ($obH2SECTIONVIDEO = $resH2SECTIONVIDEO->GetNext()) {	if(!empty($obH2SECTIONVIDEO['VALUE'])) {$isH2SECTIONVIDEO = true;}	}
...
$resH2INSTRUCTIONS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_INSTRUCTIONS"));
	while ($obH2INSTRUCTIONS = $resH2INSTRUCTIONS->GetNext()) {	if(!empty($obH2INSTRUCTIONS['VALUE'])) {$isH2INSTRUCTIONS = true;}	}
...
$resH2CERTIFICATES = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_CERTIFICATES"));
	while ($obH2CERTIFICATES = $resH2CERTIFICATES->GetNext()) {	if(!empty($obH2CERTIFICATES['VALUE'])) {$isH2CERTIFICATES = true;}	}
												...
												<div class="product-item-detail-info-container" style="<?=(!$actualItem['CAN_BUY'] ? 'display: none;' : '')?>"
													...
													<div class="product-item-amount">
														<div class="product-item-amount-field-container">
															...
															<input class="col-xs-6 product-item-amount-field" id="<?=$itemIds['QUANTITY_ID']?>" type="number"
...
<!-------------buttons--------------->
							<div class="col-xs-6 col-sm-3 product-item-detail-pay-block_1">
								...
								?>
								<?
								if(($arResult['PROPERTIES']['NALICHIE']['VALUE'] == 'В наличии') || ($arResult['PROPERTIES']['NALICHIE']['VALUE'] == '')){?><div class="nalichie"><img src="/upload/iblock/icons/checkmark_green.png">В наличии</div><?}
								else{?><div class="nalichie"><img src="/upload/iblock/icons/checkmark_gray.png"><?=$arResult['PROPERTIES']['NALICHIE']['VALUE'];?></div><?}
								?>
							</div>
...
if($arResult['ITEM_PRICES'][0]['PRICE'] > 35000){$deliveryFree='<span class="color_green"> бесплатно</span>';}
...
?>
	...
	<div class="feature feature-icon-hover indent first">
	 <span class="icon"><img src="/upload/iblock/icons/delivery_1.png"></span>
		<p class="no-margin ">
			<a href="/about/delivery/" target="_blank"><?echo "Доставим ".$share; echo $deliveryFree;?></a>
	<!--a href="/about/delivery/" target="_blank">Максимально быстрая доставка</a-->
		</p>
	</div>
	<div class="feature feature-icon-hover indent indent">
	 <span class="icon"><img src="/upload/iblock/icons/credit-card-payment.png"></span>
		<p class="no-margin ">
	...
	<div class="feature feature-icon-hover indent indent">
	 <span class="icon"></span>
		<p class="no-margin ">
			<a href="" target="_blank"><?=$arResult['PROPERTIES']['description_action']['VALUE']?></a>
		</p>
	</div>
	<?}?>
...
?>
	...
	<div class="feature feature-icon-hover indent first">
	 <span class="icon"><img src="/upload/iblock/icons/delivery_1.png"></span>
		<p class="no-margin ">
			<a href="/about/delivery/" target="_blank"><?echo "Срок поставки уточняйте.";?></a>
...
<?	
	...
	<!--/****************************************/-->
								...
								<?	}	?>
								<?	if ($isDIMENSIONS)	{	?>
										<li class="product-item-detail-tab active" data-entity="tab" data-value="dimensions">
											<a href="javascript:void(0);" class="product-item-detail-tab-link">
	...
	<!--/****************************************/-->									
				...
				<div class="row" id="<?=$itemIds['TAB_CONTAINERS_ID']?>">
					<div class="col-xs-12">
						<?	if ($isVIDEOYOUTUBE)	{	?>
								<div class="product-item-detail-tab-content" data-entity="tab-container" data-value="videoyoutube">
									<?
									/***************02_09_2019***************/
									if($isH2SECTIONVIDEO ) {
										$resH2SECTIONVIDEO = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_VIDEO"));
	...
	<!-- ================================================================================================================= -->
						<?	if ($isACCESSORIES)	{	?>
							<div class="product-item-detail-tab-content" data-entity="tab-container" data-value="accessories" >
								<div class = "col-xs-12 col-sm-5">
												...
												$resEl = CIBlockElement::GetByID($obACCESSORIES['VALUE']);
													if($ar_resEl = $resEl->GetNext())
														...
														if($ar_resElement = $resElement->GetNext())
															if($i <= $n){$j = $tegsNum[$i-1];}else{$j = $i;}
														//echo $ar_resElement['DETAIL_PAGE_URL']; <a href="/sredstva_dlya_ukhoda_za_vodoy/">
														echo $j.'. ';	echo '<a href="'.$ar_resElement['DETAIL_PAGE_URL'].'"target="_blank" style="color: #256aa3;text-decoration: underline;">'.$ar_resEl['NAME'].'</a>'; $i++;
											...
											if($arFile) 
												{	?>
													<div class = "row">
														...
														<?//if($USER->IsAdmin()) {echo '<pre>'; print_r($arFile); echo '</pre>';}?>
															<a download="<?=$arFile['ORIGINAL_NAME']?>" href="<?=$arFile['SRC']?>" target="_blank" title="Скачать" style="color: #256aa3;text-decoration: underline;">
						...
						<? }	?>
						<?	if ($isDIMENSIONS)	{	?>
							<div class="product-item-detail-tab-content" data-entity="tab-container" data-value="dimensions">
							<?/***************20_08_2019***************/
							if($isH2DIMENSIONS ) {
								$resH2DIMENSIONS = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['ID'], array("sort" => "asc"), Array("CODE"=>"SUBTITLE_SECTION_DIMENSIONS"));
						...
						<?	}	?>
						<?	if ($isRELATEDPRODUCTS)	{	?>
							<div class="product-item-detail-tab-content" data-entity="tab-container" data-value="relatedproducts">
							<?/***************20_08_2019***************/
	...
	<div class="product-item-detail-tabs-container-fixed hidden-xs" id="<?=$itemIds['TABS_PANEL_ID']?>">
		<ul class="product-item-detail-tabs-list">
			...
			<?	}	?>
			<?	if ($isRELATEDPRODUCTS)	{	?>
				<li class="product-item-detail-tab" data-entity="tab" data-value="relatedproducts">
					<a href="javascript:void(0);" class="product-item-detail-tab-link">
						<span><?=GetMessage('CP_BCE_RELATED_PRODUCTS_TAB')?></span>
					</a>
				</li>
