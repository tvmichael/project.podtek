...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
			//this.changeText(parameters.result);                      // переклад
			...
			this.kostilTranslete();                      // переклад тексту       
            this.addressDeliveryContainer = false;     
            ...
            if (action == 'saveOrderAjax') {
                ...
                BX.ajax({
                    ...
                    onsuccess: BX.delegate(function (result) {
                        ...
                        //console.log(result);
						this.changeText(result.order);                      // переклад - укр. рос. анг.
                        ...
                        // update- 18-05-18
                        $("#soa-property-3").mask("38(099) 999-9999");      // підключаємо маску на телефон 
                        loadPostOfficeCity();                               // завантажуємо відділення НовоїПошти
                        this.kostilTranslete();                             // переклад
                        this.hideDeliveryBranch();                          // показати приховати доставку
                    }, this),
        ...
        // update
        hideDeliveryBranch: function(){     // ховаємо відділення для доставки
            var hideBlock =  $('#bx-soa-delivery .bx-soa-pp-company');
            var select = $('#ID_DELIVERY_ID_34').parent().parent();

            if ($(hideBlock[1]).hasClass("bx-selected"))                
            {
                $('#bx-soa-region').hide();
                $("[data-property-id-row='67']").hide();
            } else {
                $('#bx-soa-region').show();
                $("[data-property-id-row='67']").show();
            }

            if ($(select).hasClass('bx-selected') )
                $("[data-property-id-row='67']").hide();
            else if (!$(hideBlock[1]).hasClass("bx-selected")) 
                $("[data-property-id-row='67']").show();
        },
        ...
        editActivePropsBlock: function (activeNodeMode) {            
            ...
            if (this.initialized.props) {
                //console.log('---------------- editActivePropsBlock - this.initialized.props');
                BX.remove(BX.lastChild(node));
                ...
                this.addresDeliveryProperty(propsNode);                
            }
        },

        addresDeliveryProperty: function(propsNode)         // додаємо - "Адрес доставки"
        {
            //console.log('------------------------------------------------------------');
            //console.log( $("[data-property-id-row='7']").length);

            if (!this.addressDeliveryContainer)
            {
                this.addressDeliveryContainer = $("[data-property-id-row='7']", propsNode)[0];
                if (this.addressDeliveryContainer)                    
                    $('#bx-soa-properties .bx-soa-section-content').append(this.addressDeliveryContainer);
            }
            ...
            if (this.addressDeliveryContainer)
            {
                ...
                if ($(select).hasClass('bx-selected')) 
                {
                    $(this.addressDeliveryContainer).show();
                    //$("[data-property-id-row='67']").hide();
                }
                else 
                {
                    $(this.addressDeliveryContainer).hide(); 
                    //$("[data-property-id-row='67']").show();   
                }     
            }
        },
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (disabled) {
				...
				} else if (property.Id == 67){ 
                property.Name = BX.util.htmlspecialchars(BX.message('bx-soa-custom-label-property-getId67'));
                } else if (property.Id == 7){ 
                property.Name = BX.util.htmlspecialchars(BX.message('bx-soa-custom-label-property-getId7'));
                } else { 
