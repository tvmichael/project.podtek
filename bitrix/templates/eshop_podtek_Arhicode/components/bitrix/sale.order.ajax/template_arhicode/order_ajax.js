...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
            ...
            this.phoneMask = null;
            this.obPhoneMask = BX('soa-property-3');
            if (this.obPhoneMask){
                var maskOptions = {
		...
		validateString: function(input, arProperty, fieldName)
		{
            ...
            if(arProperty.CODE == "PHONE" && !this.phoneMask){
                var maskOptions = {
                    mask: '+{7}(000)000-00-00'
                };
                this.phoneMask = new IMask(input, maskOptions);
            }
			...
			if (value.length)
			{
                if(arProperty.CODE == "PHONE" && value.length < 10)
                    errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
